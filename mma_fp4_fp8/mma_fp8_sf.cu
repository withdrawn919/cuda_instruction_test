...
struct FP8_SF_SM120_16x8x32_TN {
    ...
    void fma(float &d0, float &d1, float &d2, float &d3,
  ...
  __global__ void fp8_sf_mma_kernel(
...
int main(){
    ...
    // 创建CUDA事件计时
    // warm up
    fp8_sf_mma_kernel<<<1,32>>>(d_d,d_a,d_b,d_c,d_sfa, d_sfb);
    ...
    printf("单次fp8_sf_mma_kernel运行时间: %.12f ms\n", (float)(ms/ITERS));
    #else
